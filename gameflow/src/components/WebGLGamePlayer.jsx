...
function WebGLGamePlayer({
  ...
  const resolvedAspectRatio = aspectRatio ?? DEFAULT_ASPECT_RATIOS[mode] ?? DEFAULT_ASPECT_RATIOS.landscape;
  const aspectRatioValue = parseAspectRatio(resolvedAspectRatio) ?? (mode === 'portrait' ? 9 / 16 : 16 / 9);
  ...
  if (isFullscreen) {
    ...
    if (shouldRotateFullscreen) {
      ...
      const fittedWidth = Math.min(viewportWidth, viewportHeight * aspectRatioValue);
      frameStyle.width = `${fittedWidth}px`;
      frameStyle.height = `${fittedWidth / aspectRatioValue}px`;
    }
  }
  ...
  async function startGame() {
    ...
    try {
      ...
      if (mode === 'landscape' && window.screen.orientation?.lock) {
        try {
  ...
  return (
    <section
      ...
      style={{
        '--webgl-game-aspect-ratio': resolvedAspectRatio,
